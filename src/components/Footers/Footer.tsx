...
const Footer = () => {
  ...
  return (
    <footer className=" w-full bg-[#D1D1D1]  py-2 flex flex-col item-center mb-[-100px]">
          ...
          <div className="flex gap-4 w-full pt-3">
            {footerData.map((section, index) => (
              <div key={index} className="flex flex-col gap-4 lg:gap-5 w-full">
                <span className="text-[#2F2F2F] font-medium text-base leading-[1.6]">
                  {section.title}
                </span>
                {section.links.map((link, linkIndex) => (
                  <Link
                    ...
                    className="text-[#2F2F2F] text-sm leading-[1.3] hover:text-gray-400 transition-[.3]"
                  >
                    {link.label}
                  </Link>
                ))}
      ...
      <div className="mx-auto mb-20 px-5 flex w-full flex-col slg:hidden">
        <section className="flex flex-col justify-between gap-1 sm:gap-6 mt-2 px-2 xs:px-6 sm:px-10">
          ...
          <div className="flex lg:gap-8 w-full pt-6">
            {footerData.map((section, index) => (
              <div key={index} className="flex flex-col gap-2 sm:gap-5 w-full">
                ...
                {section.links.map((link, linkIndex) => (
                  <Link
                    key={linkIndex}
                    href={link.href}
                    className={`${link.href === pathname ? "text-gray-700" : "text-white"} text-xs sm:text-sm font-[400] hover:text-[#DC8204] transition-[.3] leading-6`}
                  >
                    {link.label}
                  </Link>
                ))}
      ...
      <div className="mx-auto max-w-[1350px]">
        <div className="flex items-center justify-center py-2">
          <div className="text-[#2F2F2F] sm:font-mono text-xs leading-[1.2]">
